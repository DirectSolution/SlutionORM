...
class ResultController extends AbstractController implements \Iterator, \ArrayAccess, \Countable {
    ...
    protected function quote($val) {
        ...
        if (is_int($val) || $val instanceof LiteralController) { // number or SQL code - for example "NOW()"
            return (string) $val;
        }
    ...
    function insert_multi(array $rows) {
        ...
        $data = reset($rows);
        $parameters = array();
        if ($data instanceof ResultController) {
        ...
        if (is_array($data)) {
            ...
            foreach ($rows as $value) {
                ...
                foreach ($value as $val) {
                    if ($val instanceof LiteralController && $val->parameters) {
                        $parameters = array_merge($parameters, $val->parameters);
                    }
                }
            }
            //! driver specific extended insert
            $insert = ($data || $this->solutionORM->driver == "mysql" ? "(" . implode(", ", array_keys($data)) . ") VALUES " . implode(", ", $values) : "DEFAULT VALUES"
                    );
        }
...
//        return;
    ...
    protected function whereOperator($operator, array $args) {
        ...
        if (count($args) != 2 || strpbrk($condition, "?:")) { // where("column < ? OR column > ?", array(1, 2))
            ...
            $condition .= " IS NULL";
        } elseif ($parameters instanceof ResultController) { // where("column", $db->$table())
            $clone = clone $parameters;
            if (!$clone->select) {
                $clone->select($this->solutionORM->structure->getPrimary($clone->table));
            }
            if ($this->solutionORM->driver != "mysql") {
                if ($clone instanceof MultiResultController) {
                ...
                foreach ($clone as $row) {
                    ...
                    if ($clone instanceof MultiResultController && count($row) > 1) {
                        array_shift($row);
                    }
                    if (count($row) == 1) {
                        $in[] = $this->quote($row[0]);
     ...
     * @return NotORM_Result fluent interface
     */
    function union(ResultController $result, $all = false) {
    ...
    protected function execute() {
        if (!isset($this->rows)) {
            ...
            foreach (array_merge($this->select, array($this, $this->group, $this->having), $this->order, $this->unionOrder) as $val) {
                if (($val instanceof LiteralController || $val instanceof self) && $val->parameters) {
                    $parameters = array_merge($parameters, $val->parameters);
                }
            }
            try {
    ...
    function fetchPairs($key, $value = '') {
        ...
        foreach ($clone as $row) {
            ...
            if ($value != "" && $clone instanceof MultiResultController) {
                array_shift($values);
            }
