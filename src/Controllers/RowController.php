...
//class RowController extends AbstractController implements \IteratorAggregate, \ArrayAccess, \Countable, \JsonSerializable {
    ...
    function __construct(array $row, ResultController $result) {
        $this->row = $row;
        $this->result = $result;
     ...
     */
    function __get($name) {
        $column = $this->result->solutionORM->structure->getReferencedColumn($name, $this->result->table);
     ...
     * @return null
     */
    function __set($name, RowController $value = null) {
        $column = $this->result->solutionORM->structure->getReferencedColumn($name, $this->result->table);
        $this[$column] = $value;
    }
    ...
    function __call($name, array $args) {
        $table = $this->result->solutionORM->structure->getReferencingTable($name, $this->result->table);
        $column = $this->result->solutionORM->structure->getReferencingColumn($table, $this->result->table);
        $return = new MultiResultController($table, $this->result, $column, $this[$this->result->primary]);
    ...
    protected function access($key, $delete = false) {
        if ($this->result->solutionORM->cache && !isset($this->modified[$key]) && $this->result->access($key, $delete)) {
